...
#include <string.h>
//#include <mylibs/ATMEGA328P/I2C.h>
#include <mylibs/ATMEGA328P/SPI.h>
#include <mylibs/ATMEGA328P/uart.h>
//#include <mylibs/ATMEGA328P/LCD-I2C.h>
//#include <mylibs/ATMEGA328P/PCF8563.h>
//#include <mylibs/ATMEGA328P/KEYPAD.h>
//#include <mylibs/ATMEGA328P/PN532_spi.h>
...
int main(void)
{
	...
	_delay_ms(150);
	buffer[0] = (1<<MASK_RX_DR|1<<MASK_TX_DS|1<<MASK_MAX_RT|1<<EN_CRC|1<<PWR_UP);	//CONFIG REGISTER
	spi_write(CONFIG,1);

	_delay_ms(2);

	buffer[0] = (1<<ENAA_P0);	// enable auto ack on pipe 0
	spi_write(EN_AA,1);

	buffer[0] = (1<<ERX_P0);	// enable rx address on pipe0
	spi_write(EN_RXADDR,1);

	buffer[0] = (1<<0|1<<1);	//5bytes address width
	spi_write(SETUP_AW,1);

	buffer[0] = 0x15;	//retransmit delay 500us,5 times on fail
	spi_write(SETUP_RETR,1);

	buffer[0] = 50;	//freq=
	spi_write(RF_CH,1);

	buffer[0] = (1<<RF_DR_HIGH|1<<1|1<<2);	// 2mbps,0dbm
	spi_write(RF_SETUP,1);

	buffer[0] = (1<<EN_DPL);	// enable dynamic payload length
	spi_write(FEATURE,1);
	
	buffer[0] = (1<<DPL_P0);	// enable dynamic payload length on pipe 0
	spi_write(DYNPD,1);
    ...
    {	
		if (!(PINC&(1<<3)))
		{
			buffer[0] = '1';	//ACTUAL D A T A
			spi_write(W_TX_PAYLOAD,1);
			PORTB |= 1<<CE;
			_delay_us(30);
			PORTB &= ~(1<<CE);	
			_delay_ms(1);
			spi_send_command(FLUSH_TX);
